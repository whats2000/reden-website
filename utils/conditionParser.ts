...
export function parseCondition(conditionString: string, t: (key: string, params?: any) => string): Condition {
  const minMatch = conditionString.match(/min\((\d+)\)/);
  if (minMatch) {
    const minValue = parseInt(minMatch[1], 10);
    return (value: number) => value >= minValue || t('litematica_generator.size_min', { size: minValue });
  }

  const maxMatch = conditionString.match(/max\((\d+)\)/);
  if (maxMatch) {
    const maxValue = parseInt(maxMatch[1], 10);
    return (value: number) => value <= maxValue || t('litematica_generator.size_max', { size: maxValue });
  }
  ...
  if (modMatch) {
      ...
      return (value: number) => value % modValue === remValue || t('litematica_generator.size_mod', { mod: modValue, rem: remValue });
    }
